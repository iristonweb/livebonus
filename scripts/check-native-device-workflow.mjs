...
function hasPullRequestWorkflowTrigger(lines) {
  ...
  for (let index = triggerLine + 1; index < lines.length; index += 1) {
    ...
    if (pathEntry && paths) paths.push(unquote(withoutInlineComment(pathEntry[1])));
  }

  return paths?.length === 1 && paths[0] === ".github/workflows/native-device-release.yml";
}
...
export function validateNativeDeviceWorkflow(source) {
  ...
  if (!contractJob) {
    ...
    if (!contractJob.some((line) => /^\s+runs-on:\s*ubuntu-latest\s*$/.test(line))) {
      errors.push("workflow-contract must run on ubuntu-latest");
    }
    if (
      !contractJob.some(
        (line) => /^\s+run:\s*node scripts\/check-native-device-workflow\.mjs\s*$/.test(line),
      )
    ) {
      errors.push("workflow-contract must run the native workflow checker directly with node");
    }
    if (contractJob.some((line) => /\bpnpm\b|secrets\./.test(line))) {
      errors.push("workflow-contract must not require pnpm or secrets");
    }
  }
  ...
  if (!releaseJob) {
    ...
    if (!releaseJob.some((line) => /^\s+needs:\s*workflow-contract\s*$/.test(line))) {
...
export async function checkNativeDeviceWorkflow(
